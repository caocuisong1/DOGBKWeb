...
def search():
    ...
    with connection.cursor() as cursor:
        sql = "SELECT * FROM DOGBK WHERE name LIKE %s OR alias LIKE %s OR size LIKE %s"
        cursor.execute(sql, ("%" + keywords + "%", "%" + keywords + "%", "%" + keywords + "%"))
...
def get_images():
    ...
    # Select query
    query = "SELECT img1, img2, img3 FROM DOGBK"
    ...
    return render_template('img.html', rows=rows)
